...
impl helix_event::AsyncHook for CopilotHandler {
    ...
    fn finish_debounce(&mut self) {
        ...
        dispatch_blocking(move |editor, _| {
            copilot_completion(editor, token);
        });
    }
}

fn copilot_completion(editor: &mut Editor, cancel: TaskHandle) {
                ...
                dispatch(move |editor, _| {
                    ...
                    let doc_completions = completions
                        ...
                        .filter_map(|completion| {
                            ...
                            */
                            let mut txt_fmt = doc.text_format(view.inner_width(&doc), None);
                            // Match the forced soft-wrap used when rendering the suggestion so the
                            // reserved virtual-line count agrees with the wrapped rendering.
                            txt_fmt.soft_wrap = txt_fmt.viewport_width > 10;

                            let Some(range) =
                                lsp_range_to_range(doc.text(), completion.range, offset_encoding)
                            else {
                                return None;
                            };
                            let (start_coords, end_coords) = (
                                coords_at_pos(doc.text().slice(..).into(), range.anchor),
                                coords_at_pos(doc.text().slice(..).into(), range.head),
                            );
                    ...
                    doc.copilot
                        .fill_with_completions(doc_completions, offset_encoding);
                })
                .await;
            }
        }
...
pub(super) fn try_register_hooks(handlers: &Handlers) {
    let Some(copilot_handler) = handlers.copilot.clone() else {
        return;
    };
